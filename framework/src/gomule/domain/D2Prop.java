...
package gomule.domain;

import gomule.d2Files.D2TblFile;
import gomule.d2Files.D2TxtFile;
    ...
    public String generateDisplay( int qFlag, int cLvl ) {
        ...
        switch( funcN ) {
            ...
            case ( 35 ):
                ...
                if( pVals[0] == pVals[1] ) {
                    return "Adds " + pVals[0] + " Cold Damage Over " + Math.round( pVals[2] / 25.0 ) + " Secs (" + pVals[2] + " Frames)";
                }

                return "Adds " + pVals[0] + " - " + pVals[1] + " Cold Damage Over " + Math.round( pVals[2] / 25.0 ) + " Secs (" + pVals[2] + " Frames)";
            ...
            case ( 36 ):
                ...
                if( pVals.length == 4 ) {
                    ...
                    if( pVals[0] == pVals[1] ) {
                        return "Adds " + Math.round( pVals[0] * (double)pVals[2] / 256 ) + " Poison Damage Over " + (int)Math.floor( pVals[2] / 25.0 ) + " Secs (" + pVals[2] + " Frames)";
                    }

                    return "Adds " + Math.round( pVals[0] * (double)pVals[2] / 256 ) + " - " + Math.round( pVals[1] * (double)pVals[2] / 256 ) + " Poison Damage Over " + (int)Math.floor( pVals[2] / 25.0 ) + " Secs (" + pVals[2] + " Frames)";
                }
            case ( 37 ):
        ...
        switch( op ) {
            ...
            case ( 5 ):
                ...
                if( D2TxtFile.ITEM_STAT_COST.getRow( pNum ).get( "op base" ).equals( "level" ) ) {
                    ...
                    pVals[0] = (int)Math.floor( ( pVals[0] * cLvl ) / ( Math.pow( 2, Integer.parseInt( D2TxtFile.ITEM_STAT_COST.getRow( pNum ).get( "op param" ) ) ) ) );
                }
        }
        opApplied = true;
    }
